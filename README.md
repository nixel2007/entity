...
* <a href="#header">entity - OneScript Persistence API</a>
  ...
  * <a href="#observers">Наблюдатели источника данных</a>
...
```bsl
// Описание источника данных. Коннектором к базе выступает референсная реализация КоннекторSQLite.
// В качестве БД используется "база в оперативной памяти".
Источник = Новый ИсточникДанных("Основной", Тип("КоннекторSQLite"), "FullUri=file::memory:?cache=shared");

// Создание менеджера сущностей из источника данных.
...
Источник данных, с которым работает менеджер сущностей, сообщает зарегистрированным наблюдателям
обо всем, что с ним происходит: об операциях (Сохранить, Получить, Удалить и других), запросах к
СУБД, захвате и освобождении соединений и транзакциях. Каждое событие приходит наблюдателю дважды -
до выполнения и после - одним объектом `СобытиеИсточникаДанных`, в котором после выполнения есть
длительность, ошибка и результат. На этом строятся телеметрия, журнал медленных запросов, подсчет
обращений к БД в тестах.

Наблюдатель объявляет интерфейс `НаблюдательИсточникаДанных` аннотацией на конструкторе и
...
Процедура ПослеСобытия(Событие) Экспорт
    Если Событие.Вид() = ВидыСобытийИсточникаДанных.Запрос() И Событие.Длительность() > 100 Тогда
        Сообщить(СтрШаблон("Медленный запрос к %1: %2 мс", Событие.ИмяТаблицы(), Событие.Длительность()));
    КонецЕсли;
КонецПроцедуры
```

Регистрируется наблюдатель на источнике данных, а менеджер создается из источника и отдает
наблюдателю все, что с источником происходит: операции самого менеджера, его хранилищ сущностей
и активной записи, запросы, соединения и транзакции.

```bsl
Источник = Новый ИсточникДанных("Основной", Тип("КоннекторSQLite"), СтрокаСоединения);
Источник.ДобавитьНаблюдателя(Новый МедленныеЗапросы);
МенеджерСущностей = Новый МенеджерСущностей(Источник);
```

Виды событий, их поля, поведение при ошибках и в нескольких потоках, а также поддержка
наблюдателей в собственном коннекторе описаны в разделе
[Наблюдатели источника данных](docs/Наблюдатели.md).
...
МенеджерСущностей предоставляет публичный интерфейс по чтению, сохранению, удалению данных. МенеджерСущностей создается из ИсточникДанных, который задает тип *коннектора* к используемой базе данных, строку соединения и размер пула и владеет пулом соединений; других способов создать менеджер нет. Все операции по изменению данных МенеджерСущностей делегирует Коннектору. В зоне ответственности МенеджераСущностей находятся:
...
ХранилищеСущностей предоставляет тот же интерфейс по работе с сущностями и транзакциями, но с привязкой к конкретному типу сущности. Для получения ХранилищаСущностей служит метод `МенеджерСущностей::ПолучитьХранилищеСущностей`.

В отличие от МенеджераСущностей, ХранилищеСущностей не требует передачи в методы параметра "ТипСущности".

Своей логики работы с БД у хранилища нет: оно подставляет свой тип сущности и делегирует операцию менеджеру, которому принадлежит. Поэтому поведение вызова через хранилище и через менеджер совпадает, в том числе для транзакций и прямого доступа к БД.

Хранилища сущностей и пулы сущностей принадлежат менеджеру сущностей: хранилище работает с моделью данных того менеджера, который его создал, и закрывается вместе с ним. Один менеджер на один тип сущности всегда возвращает одно и то же хранилище, а два менеджера, созданные из одного источника данных, возвращают разные хранилища со своими пулами сущностей. Соединения при этом общие: их дает пул источника.
...
Коннектор **может** дополнительно реализовать интерфейс [`НаблюдаемыйКоннектор`](docs/НаблюдаемыйКоннектор.md): тогда наблюдатели источника данных получают события его запросов к СУБД и описание соединения. Встроенные коннекторы реализуют оба интерфейса. Коннектор без второго интерфейса работает как раньше.

> Важно!

Менеджеры и хранилища сущностей одного источника данных собственных коннекторов не держат: они берут соединения из общего пула этого источника. Тип, строка соединения и параметры коннектора задаются при создании ИсточникаДанных.
...
База данных в памяти живет ровно столько, сколько открыто хотя бы одно соединение к ней. Как только закрывается последнее, база уничтожается вместе с таблицами и данными.

Поэтому in-memory базу нельзя сочетать с **нулевым размером пула соединений**: при нулевом размере соединение открывается на операцию и закрывается сразу после нее, то есть между операциями открытых соединений не остается. Следующая операция откроет уже пустую базу и получит `no such table`. Переживет база такой промежуток или нет, зависит от внутреннего пулинга провайдера, то есть воспроизводится через раз.

Для in-memory базы используйте ненулевой размер пула — тогда пул придерживает соединения открытыми и база живет, пока не закрыт источник данных. Если нулевой размер нужен принципиально, держите отдельное соединение к той же базе на все время работы:

```bsl
Держатель = Новый КоннекторSQLite;
Держатель.Открыть("FullUri=file::memory:?cache=shared", Новый Массив);
// ... работа через менеджер сущностей ...
Держатель.Закрыть();
```
...
* для Major.Entity:
  * все публичные непомеченные как "нестабильные" (`@unstable`) или "для служебного использования" (`@internal`) методы классов:
    ...
    * [`НаблюдательИсточникаДанных`](src/Классы/НаблюдательИсточникаДанных.os);
    * [`СобытиеИсточникаДанных`](src/Классы/СобытиеИсточникаДанных.os);
  * значения модулей-перечислений:
    ...
    * [`ВидыСобытийИсточникаДанных`](src/Модули/ВидыСобытийИсточникаДанных.os);
...
* для Major.Connector:
  ...
  * все публичные методы необязательного интерфейса [`НаблюдаемыйКоннектор`](src/Классы/НаблюдаемыйКоннектор.os) и их сигнатуры, а также методы `НачатьЗапрос` и `ЗавершитьЗапрос` реестра наблюдателей, который коннектор получает в `УстановитьНаблюдателей`;
