...
* <a href="#header">entity - OneScript Persistence API</a>
  ...
  * <a href="#observers">Наблюдатели сущностей</a>
...
Менеджер сущностей сообщает зарегистрированным наблюдателям о своей работе: об операциях
(Сохранить, Получить, Удалить и других), запросах к СУБД, захвате и освобождении соединений пула
и транзакциях. Каждое событие приходит наблюдателю дважды - до выполнения и после - одним объектом
`СобытиеСущностей`, в котором после выполнения есть длительность, ошибка и результат. На этом
строятся телеметрия, журнал медленных запросов, подсчет обращений к БД в тестах.

Наблюдатель объявляет интерфейс `НаблюдательСущностей` аннотацией на конструкторе и реализует два
метода:
...
Регистрируется наблюдатель на менеджере и видит все, что идет через его пул: операции самого
менеджера, его хранилищ сущностей и активной записи.
...
[Наблюдатели сущностей](docs/Наблюдатели.md).
...
Коннектор **может** дополнительно реализовать интерфейс [`НаблюдаемыйКоннектор`](docs/НаблюдаемыйКоннектор.md): тогда наблюдатели сущностей получают события его запросов к СУБД и описание соединения. Встроенные коннекторы реализуют оба интерфейса. Коннектор без второго интерфейса работает как раньше.
...
Для in-memory базы используйте ненулевой размер пула — тогда пул придерживает соединения открытыми и база живет, пока жив менеджер сущностей. Если нулевой размер нужен принципиально, держите отдельное соединение к той же базе на все время работы:
...
* для Major.Entity:
  * все публичные непомеченные как "нестабильные" (`@unstable`) или "для служебного использования" (`@internal`) методы классов:
    ...
    * [`НаблюдательСущностей`](src/Классы/НаблюдательСущностей.os);
    * [`СобытиеСущностей`](src/Классы/СобытиеСущностей.os);
  * значения модулей-перечислений:
    ...
    * [`ВидыСобытийСущностей`](src/Модули/ВидыСобытийСущностей.os);
