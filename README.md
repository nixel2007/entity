...
МенеджерСущностей предоставляет публичный интерфейс по чтению, сохранению, удалению данных. МенеджерСущностей создается из ИсточникДанных, который описывает тип *коннектора* к используемой базе данных, строку соединения и размер пула; создание менеджера напрямую по параметрам соединения устарело и сохранено для совместимости. Все операции по изменению данных МенеджерСущностей делегирует Коннектору. В зоне ответственности МенеджераСущностей находятся:
