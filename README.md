...
* <a href="#header">entity - OneScript Persistence API</a>
  ...
  * <a href="#schema-migrations">Миграции схемы данных</a>
    * <a href="#schema-migrations-handlers">Обработчики миграции данных</a>
    * <a href="#schema-migrations-versions">Версия схемы и конвейер миграций</a>
    * <a href="#schema-migrations-barriers">Барьерные версии</a>
  ...
  * <a href="#library-structure">Структура библиотеки</a>
    ...
    * <a href="#library-structure-schema-migrator">МиграторСхемы</a>
...
Множество значений для видов сравнения `В` и `НеВ` передается массивом. Подробнее -
в описании [`ВидСравнения`](docs/ВидСравнения.md#отбор-по-множеству-значений).
...
<a id="schema-migrations" />

## Миграции схемы данных

При вызове `МенеджерСущностей.Инициализировать()` библиотека автоматически приводит схему БД
к состоянию модели данных. Для каждой зарегистрированной сущности (включая подчиненные таблицы)
запрашивается фактическое описание таблицы, сравнивается с моделью и строится план миграции:

* отсутствующие таблицы создаются;
* отсутствующие колонки добавляются;
* колонки с изменившимся типом приводятся к новому типу (SQLite выполняет перестроение таблицы
  с переносом данных, PostgreSQL - `ALTER TABLE ... ALTER COLUMN`);
* колонки, отсутствующие в модели, по умолчанию **не удаляются** - в журнал выводится
  предупреждение. Удаление включается опцией `УстановитьУдалениеЛишнихКолонок`.

Механизм не зависит от конкретной СУБД: провайдероспецифичная часть описана интерфейсом
[АбстрактныйКоннекторМиграций](docs/АбстрактныйКоннекторМиграций.md), который реализуют все
коннекторы библиотеки. Сторонний коннектор без поддержки этого интерфейса продолжает работать
по-старому: для каждой таблицы вызывается `ИнициализироватьТаблицу`.

Живой пример трех версий мини-приложения с миграциями, барьерной версией и сценариями
запуска — каталог [demo](demo).

Настройка миграций выполняется через [МиграторСхемы](docs/МиграторСхемы.md) до вызова
`Инициализировать()`:
...
МиграторСхемы.УстановитьУдалениеЛишнихКолонок(Истина); // по умолчанию - Ложь

МенеджерСущностей.Инициализировать(); // анализ схемы и применение изменений
```

<a id="schema-migrations-handlers" />

### Обработчики миграции данных

При обновлении схемы часто требуется преобразовать данные: заполнить новую колонку, перенести
значения из удаляемой колонки, нормализовать справочники. Для этого регистрируются обработчики
миграции - произвольные объекты, реализующие методы `ПередМиграцией` и/или `ПослеМиграции`
(см. [АбстрактныйОбработчикМиграции](docs/АбстрактныйОбработчикМиграции.md)):
...
```bsl
МенеджерСущностей.ДобавитьОбработчикМиграции(Новый ЗаполнениеРейтингаАвторов);
МенеджерСущностей.Инициализировать();
```

`ПередМиграцией` вызывается до применения изменений схемы (например, чтобы прочитать данные
удаляемой колонки), `ПослеМиграции` - после. Оба метода получают менеджер сущностей и
[ПланМиграции](docs/ПланМиграции.md) с перечнем изменений. Обработчики без привязки к версии
вызываются при каждом обновлении схемы.

<a id="schema-migrations-versions" />

### Версия схемы и конвейер миграций

Приложение может объявить версию своей схемы данных. В этом случае текущая версия хранится
в служебной таблице `entity_ВерсияСхемы`, а обработчики миграции можно привязывать к версиям,
реализовав метод `ВерсияСхемы()`:

```bsl
МенеджерСущностей.УстановитьВерсиюСхемы("0.3.0");
МенеджерСущностей.ДобавитьОбработчикМиграции(Новый МиграцияДанных_0_2_0); // ВерсияСхемы() = "0.2.0"
МенеджерСущностей.ДобавитьОбработчикМиграции(Новый МиграцияДанных_0_3_0); // ВерсияСхемы() = "0.3.0"
МенеджерСущностей.Инициализировать();
```

Правила выполнения конвейера:

* обработчик версии `V` выполняется, только если `ХранимаяВерсия < V <= ВерсияПриложения`;
* между версиями допустимо несколько миграций: при обновлении через несколько версий
  (например, с `0.1.0` сразу на `0.3.0`) выполняются все попавшие в интервал обработчики
  в порядке возрастания версий - `0.2.0`, затем `0.3.0` - независимо от порядка регистрации;
* после выполнения обработчиков каждой промежуточной версии эта версия фиксируется в БД,
  поэтому конвейер, прерванный на версии `0.3.0`, при повторном запуске продолжится с нее,
  а не начнет заново с `0.2.0`;
* при первой инициализации (в БД еще нет записи о версии) версия приложения просто фиксируется,
  версионные обработчики не выполняются;
* понижение версии не поддерживается: если версия приложения меньше хранимой,
  инициализация завершается исключением;
* схема БД приводится к целевому состоянию за один шаг, поэтому обработчики пишутся
  против актуальной модели данных.

<a id="schema-migrations-barriers" />

#### Барьерные версии: когда сквозное обновление невозможно

Иногда сквозное обновление через несколько версий приводит к потере данных. Классический пример:

* в версии `1.0.0` есть колонка `Имя`;
* в версии `2.0.0` добавлена колонка `ПолноеИмя`, обработчик миграции `2.0.0` заполняет ее из `Имя`;
* в версии `3.0.0` колонка `Имя` удалена из модели.

При сквозном обновлении `1.0.0 → 3.0.0` миграция данных версии `2.0.0` неисполнима: модель
данных приложения `3.0.0` уже не содержит колонку `Имя`, прочитать ее через сущности нельзя.
Хранить промежуточные версии схемы механизм не может, поэтому такая цепочка миграций
объявляется прерывающейся вручную - через барьерную версию:

```bsl
// Приложение версии 3.0.0: обновление поддерживается только с версии 2.0.0 и выше
МиграторСхемы = МенеджерСущностей.ПолучитьМиграторСхемы();
МиграторСхемы.УстановитьВерсиюСхемы("3.0.0");
МиграторСхемы.УстановитьМинимальнуюВерсиюОбновления("2.0.0");
```

Если хранимая версия схемы меньше барьерной, инициализация завершается исключением с указанием
выполнить обновление через промежуточную версию приложения: пользователь на версии `1.0.0`
сначала разворачивает приложение `2.0.0` (миграция `1.0.0 → 2.0.0` заполняет `ПолноеИмя`),
затем `3.0.0` (удаляется `Имя`). Обработчик миграции `2.0.0` в приложении `3.0.0` при этом
можно удалить - выполниться он уже не может.

Барьер можно объявить и на уровне отдельного обработчика методом `МинимальнаяВерсияСхемы()`:
такой обработчик при попытке участвовать в обновлении с более ранней хранимой версии
останавливает инициализацию. Это удобно, когда знание об ограничении принадлежит конкретной
миграции, а не приложению в целом:

```bsl
// file: МиграцияДанных_3_0_0.os
Функция ВерсияСхемы() Экспорт
	Возврат "3.0.0";
КонецФункции

Функция МинимальнаяВерсияСхемы() Экспорт
	Возврат "2.0.0"; // выполнение допустимо только с хранимой версии 2.0.0 и выше
КонецФункции
```

Для точечных случаев доступен и обходной путь без остановки обновления: метод `ПередМиграцией`
вызывается до применения изменений схемы, когда данные удаляемых колонок еще находятся в БД,
и может вычитать их напрямую через коннектор (например, методом `ВыполнитьЗапрос` SQL-коннекторов),
чтобы перенести в новые колонки в `ПослеМиграции`.
...
<a id="library-structure-schema-migrator" />

### МиграторСхемы

Публичный интерфейс класса - [МиграторСхемы](docs/МиграторСхемы.md).

Класс `МиграторСхемы` выполняет автоматическую миграцию схемы БД при инициализации менеджера сущностей: сравнивает модель данных с фактической схемой БД, строит [план миграции](docs/ПланМиграции.md) и применяет его через коннектор, реализующий интерфейс [АбстрактныйКоннекторМиграций](docs/АбстрактныйКоннекторМиграций.md). Поддерживает регистрацию [обработчиков миграции данных](docs/АбстрактныйОбработчикМиграции.md), хранение версии схемы в служебной таблице и конвейер версионных миграций. Подробнее см. <a href="#schema-migrations">Миграции схемы данных</a>.
...
* для Major.Entity:
  * все публичные непомеченные как "нестабильные" (`@unstable`) или "для служебного использования" (`@internal`) методы классов:
    ...
    * [`МиграторСхемы`](src/Классы/МиграторСхемы.os),
    * [`ПланМиграции`](src/Классы/ПланМиграции.os),
    * [`ИзменениеСхемы`](src/Классы/ИзменениеСхемы.os),
    * [`ОписаниеТаблицы`](src/Классы/ОписаниеТаблицы.os);
  * значения модулей-перечислений:
    ...
    * [`ВидыИзмененийСхемы`](src/Модули/ВидыИзмененийСхемы.os);
  * состав и параметры аннотаций сущностей;
  * контракт обработчиков миграции данных (см. [`АбстрактныйОбработчикМиграции`](src/Классы/АбстрактныйОбработчикМиграции.os));
...
* для Major.Connector:
  ...
  * все публичные непомеченные как "нестабильные" (`@unstable`) методы классов:
    ...
    * [`ЭлементОтбора`](src/Классы/ЭлементОтбора.os);
    * [`ЭлементПорядка`](src/Классы/ЭлементПорядка.os);
  * все публичные методы класса [`АбстрактныйКоннекторМиграций`](src/Классы/АбстрактныйКоннекторМиграций.os) и их сигнатуры;
  * значения модулей-перечислений:
    * [`ТипыКолонок`](src/Модули/ТипыКолонок.os),
    * [`ТипыПодчиненныхТаблиц`](src/Модули/ТипыПодчиненныхТаблиц.os),
    * [`ВидСравнения`](src/Модули/ВидСравнения.os),
