...
МенеджерСущностей предоставляет публичный интерфейс по чтению, сохранению, удалению данных. МенеджерСущностей создается из ИсточникДанных, который задает тип *коннектора* к используемой базе данных, строку соединения и размер пула и владеет пулом соединений; создание менеджера напрямую по параметрам соединения устарело и сохранено для совместимости. Все операции по изменению данных МенеджерСущностей делегирует Коннектору. В зоне ответственности МенеджераСущностей находятся:
...
Хранилища сущностей и пулы сущностей совпадают в рамках одного типа сущности и одного источника данных. Другими словами, два менеджера сущностей, созданные из одного источника данных, вернут одинаковые хранилища сущностей одного типа.
