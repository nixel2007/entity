...
Хранилища сущностей и пулы сущностей принадлежат менеджеру сущностей: хранилище работает моделью данных того менеджера, который его создал, и закрывается вместе с ним. Один менеджер на один тип сущности всегда возвращает одно и то же хранилище, а два менеджера, созданные из одного источника данных, возвращают разные хранилища со своими пулами сущностей. Соединения при этом общие: их дает пул источника.
...
Менеджеры и хранилища сущностей одного источника данных собственных коннекторов не держат: они работают общим пулом соединений этого источника. Тип, строка соединения и параметры коннектора задаются при создании ИсточникаДанных.
