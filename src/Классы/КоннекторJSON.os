...
//
// Чтение файла таблицы выполняется под тем же семафором, что и запись, поэтому одновременные
// чтение и запись одной таблицы из разных потоков одного процесса не конфликтуют за файл.
//
// Параметры:
//   ОбъектМодели - ОбъектМодели - Объект, содержащий описание класса-сущности и настроек таблицы БД.
...
Функция НайтиСтрокиВТаблице(ОбъектМодели, ОпцииПоиска) Экспорт
	...
	Попытка
		...
		НайденныеСтроки = НайтиСтрокиВТаблицеБезЗахватаСемафора(ОбъектМодели, ОпцииПоиска);
...
// Осуществляет поиск строк в таблице по указанному отбору, не захватывая семафор таблицы.
//
// Вызывающий код обязан удерживать семафор файла таблицы на время вызова: семафор не является
// реентерабельным, и повторный захват из потока, уже владеющего им, приведет к взаимоблокировке.
//
// Параметры:
//   ОбъектМодели - ОбъектМодели - Объект, содержащий описание класса-сущности и настроек таблицы БД.
...
//     значением элемента соответствия - значение колонки.
//
Функция НайтиСтрокиВТаблицеБезЗахватаСемафора(ОбъектМодели, ОпцииПоиска)
