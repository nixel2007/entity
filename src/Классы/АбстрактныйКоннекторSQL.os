...
Перем Лог;
Перем Коннектор;
// НаблюдателиИсточникаДанных - Наблюдатели пула, которым сообщается о каждом запросе. Неопределено,
...
//
// Параметры:
//   ТекстЗапроса - Строка - Текст запроса или команды.
//
// Возвращаемое значение:
//   Строка - Операция в верхнем регистре; для CREATE, DROP и ALTER вместе со вторым словом.
//
Функция ОперацияИзТекста(Знач ТекстЗапроса)
	...
	ОперацияНадСхемой = Операция = "CREATE" ИЛИ Операция = "DROP" ИЛИ Операция = "ALTER";
	Если ОперацияНадСхемой И Слова.Количество() > 1 Тогда
