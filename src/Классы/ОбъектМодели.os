...
Перем Идентификатор;
Перем ФиксированныйИдентификатор;

// Строки таблицы Колонки по имени поля и по имени колонки. Строятся в конструкторе и дальше только
// читаются, в том числе из многих потоков. Ссылаются на сами строки таблицы, поэтому правка строки
// через Служебный_Колонки() видна и через них; строки после конструктора не добавляются.
Перем КолонкиПоИмениПоля;
Перем КолонкиПоИмениКолонки;

// Соответствие типа колонки модели его ОписаниюТипов. Строится в конструкторе и дальше только читается.
Перем ОписанияТиповКолонок;
...
Процедура ПриСозданииОбъекта(ПТипСущности, ПМодельДанных)
	...
	ОписанияТиповКолонок = СоответствиеТиповМоделиОписанийТипов();
	ЗаполнитьКолонки();
...
Функция Идентификатор() Экспорт
	Если ФиксированныйИдентификатор = Неопределено Тогда
		Возврат Новый ФиксированнаяСтруктура(Идентификатор);
	КонецЕсли;
	Возврат ФиксированныйИдентификатор;
КонецФункции
...
Функция ПолучитьПриведенноеЗначениеПоля(Сущность, ИмяПоля) Экспорт
	Колонка = КолонкаПоИмениПоля(ИмяПоля);
...
Процедура УстановитьЗначениеКолонкиВПоле(Сущность, ИмяКолонки, ЗначениеПоля) Экспорт
	...
	Колонка = КолонкаПоИмениКолонки(ИмяКолонки);
...
Функция КолонкаПоИмениПоля(Знач ИмяПоля)
	Колонка = КолонкиПоИмениПоля.Получить(ИмяПоля);
	Если Колонка = Неопределено Тогда
		ВызватьИсключение СтрШаблон("В модели сущности %1 нет поля %2", ТипСущности, ИмяПоля);
	КонецЕсли;
	Возврат Колонка;
КонецФункции

Функция КолонкаПоИмениКолонки(Знач ИмяКолонки)
	Колонка = КолонкиПоИмениКолонки.Получить(ИмяКолонки);
	Если Колонка = Неопределено Тогда
		ВызватьИсключение СтрШаблон("В модели сущности %1 нет колонки %2", ТипСущности, ИмяКолонки);
	КонецЕсли;
	Возврат Колонка;
КонецФункции
...
Процедура ЗаполнитьКолонки()
	...
	// Как Колонки.Найти: при совпадении имен находится первая строка.
	КолонкиПоИмениПоля = Новый Соответствие;
	КолонкиПоИмениКолонки = Новый Соответствие;
	Для Каждого Колонка Из Колонки Цикл
		Если КолонкиПоИмениПоля.Получить(Колонка.ИмяПоля) = Неопределено Тогда
			КолонкиПоИмениПоля.Вставить(Колонка.ИмяПоля, Колонка);
		КонецЕсли;
		Если КолонкиПоИмениКолонки.Получить(Колонка.ИмяКолонки) = Неопределено Тогда
			КолонкиПоИмениКолонки.Вставить(Колонка.ИмяКолонки, Колонка);
		КонецЕсли;
	КонецЦикла;

	Если Идентификатор <> Неопределено Тогда
		ФиксированныйИдентификатор = Новый ФиксированнаяСтруктура(Идентификатор);
	КонецЕсли;
...
Процедура ЗаполнитьПодчиненныеТаблицы()
	...
	Для Каждого Свойство Из ТаблицаСвойств Цикл
		...
		ДанныеОПодчиненнойТаблице.ТипЭлемента = ПривестиТипЭлементаПодчиненнойТаблицы(ТипЭлемента);
...
// Определяет, в каком виде хранить тип элемента подчиненной таблицы.
//
// Имена примитивных типов модели остаются строками. Приведение их к "Тип" теряет модельное
// имя: обратное преобразование выполняется через "Строка", а строковое представление
// платформенного типа зависит от локали процесса - у таких типов есть англоязычный алиас,
// и на не-русской локали "Тип(""Строка"")" превращается в "String", которого нет среди
// имен типов модели. Кроме того, имена "Целое" и "Дробное" платформенными типами не
// являются, и приведение к "Тип" на них завершилось бы ошибкой.
//
// Ссылки на классы-сущности, наоборот, хранятся значением типа "Тип": именно им ключуется
// хранилище объектов модели данных.
//
// Параметры:
//   ТипЭлемента - Строка - Имя типа элемента из аннотации подчиненной таблицы.
//                - Неопределено - Тип элемента не указан.
//
// Возвращаемое значение:
//   Строка - Имя примитивного типа модели.
//   Тип - Тип класса-сущности.
//   Неопределено - Тип элемента не указан.
//
Функция ПривестиТипЭлементаПодчиненнойТаблицы(Знач ТипЭлемента)

	Если ТипЭлемента = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;

	Если ТипыКолонок.ЭтоПримитивныйТип(ТипЭлемента) Тогда
		Возврат ТипЭлемента;
	КонецЕсли;

	Возврат Тип(ТипЭлемента);
...
Функция ВыполнитьПриведениеЗначения(Колонка, Значение)
	...
	ОписаниеТипов = ОписанияТиповКолонок.Получить(Колонка.ТипКолонки);
