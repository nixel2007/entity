...
Процедура ЗаполнитьКолонки()
	...
	Для Каждого Свойство Из ТаблицаСвойств Цикл
		...
		ЗаполнитьТипЭлемента(ДанныеОКолонке, ДанныеОКолонке.ТипКолонки, АннотацияКолонка);
...
Процедура ЗаполнитьПодчиненныеТаблицы()
	...
	Для Каждого Свойство Из ТаблицаСвойств Цикл
		...
		ЗаполнитьТипЭлемента(ДанныеОПодчиненнойТаблице, ДанныеОПодчиненнойТаблице.ТипТаблицы, АннотацияПодчиненнаяТаблица);
...
Процедура ЗаполнитьТипЭлемента(ДанныеОКолонке, ТипКолонки, АннотацияКолонка)
	
	ЗначениеПоУмолчанию = Неопределено;
	ТипыКолонокСЭлементами = Новый Массив;
	ТипыКолонокСЭлементами.Добавить(ТипыКолонок.Ссылка);
	ТипыКолонокСЭлементами.Добавить(ТипыПодчиненныхТаблиц.Массив);
	ТипыКолонокСЭлементами.Добавить(ТипыПодчиненныхТаблиц.Структура);
	// TODO:
	// ТипыКолонокСЭлементами.Добавить(ТипыКолонок.Таблица);
	
	Если АннотацияКолонка = Неопределено ИЛИ АннотацияКолонка.Параметры = Неопределено Тогда
		ТипЭлемента = ЗначениеПоУмолчанию;
	ИначеЕсли ТипыКолонокСЭлементами.Найти(ТипКолонки) = Неопределено Тогда
		ТипЭлемента = ЗначениеПоУмолчанию;
	Иначе
