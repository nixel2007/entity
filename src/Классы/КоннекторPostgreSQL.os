	...
	Иначе
		Для Каждого ДанныеОКолонке Из КолонкиТаблицы Цикл
			...
			ЭтоПустаяСсылка = Истина
				И ДанныеОКолонке.ТипКолонки = ТипыКолонок.Ссылка 
				И (ЗначениеПараметра = 0 Или ЗначениеПараметра = Неопределено);

			ЭтоПустыеДвоичныеДанные = Истина
				И ДанныеОКолонке.ТипКолонки = ТипыКолонок.ДвоичныеДанные
				И Не ЗначениеЗаполнено(ЗначениеПараметра);

			ЭтоДвоичныеДанные = Истина
				И ДанныеОКолонке.ТипКолонки = ТипыКолонок.ДвоичныеДанные
				И ЗначениеЗаполнено(ЗначениеПараметра);

			НулевойПараметр = ЭтоПустаяСсылка Или ЭтоПустыеДвоичныеДанные;

			ИменаКолонок = ИменаКолонок + Символы.Таб + ДанныеОКолонке.ИмяКолонки + "," + Символы.ПС;
			
			Если ЭтоДвоичныеДанные Тогда
				Хекс = ПолучитьHexСтрокуИзДвоичныхДанных(ЗначениеПараметра);
				ИмяКолонки = СтрШаблон("decode('%1','hex')", Хекс);
			Иначе
				ИмяКолонки = "@" + ДанныеОКолонке.ИмяКолонки;
			КонецЕсли;

			ЗначенияКолонок = ЗначенияКолонок 
				+ Символы.Таб 
				+ ?(НулевойПараметр, "null", ИмяКолонки) 
				+ "," + Символы.ПС;

			Если Не ДанныеОКолонке.Идентификатор И Не ЭтоДвоичныеДанные Тогда

				ЗначениеОбновления = ?(НулевойПараметр, "null", "@" + ДанныеОКолонке.ИмяКолонки);
				СтрокаОбновления = СтрокаОбновления + СтрШаблон(" %1 = %2,", ДанныеОКолонке.ИмяКолонки, ЗначениеОбновления); 
			КонецЕсли;

			Если Не НулевойПараметр И Не ЭтоДвоичныеДанные Тогда
