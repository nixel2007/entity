...
Процедура Сохранить(ОбъектМодели, Сущность) Экспорт
	...
	Иначе
		Для Каждого ДанныеОКолонке Из КолонкиТаблицы Цикл
			...
			Если Не ДанныеОКолонке.Идентификатор Тогда
				...
				//СтрокаОбновления + " " + ДанныеОКолонке.ИмяКолонки + " = " + "@" + ДанныеОКолонке.ИмяКолонки + ","; 
			КонецЕсли;
	...
	ТекстЗапроса = "INSERT INTO %1 (
		...
		|%3
		|)
		|ON CONFLICT (" + КолонкаИдентификатор + ") DO UPDATE SET %4";
	
	ТекстЗапроса = СтрШаблон(ТекстЗапроса, ИмяТаблицы, ИменаКолонок, ЗначенияКолонок, СтрокаОбновления);
...
Функция СоответствиеТиповМоделиИТиповКолонок()
	...
	Карта.Вставить(ТипыКолонок.Время, "date");
	Карта.Вставить(ТипыКолонок.ДатаВремя, "date");
