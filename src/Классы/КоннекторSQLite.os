...
Процедура ИнициализироватьТаблицу(ОбъектМодели) Экспорт
	...
	Для Каждого Колонка Из КолонкиТаблицы Цикл
		...
		ИначеЕсли Колонка.ТипКолонки = ТипыКолонок.Структура Тогда
			ТипКолонкиКлюч = ТипыКолонок.Строка;
		Иначе
			ВызватьИсключение "Неизвестный тип элемента " + Колонка.ТипЭлемента;
		КонецЕсли;
		
		// TODO: Вынести заполнение в менеджер/билдер.
		ОбъектМоделиМассив = Новый ОбъектМодели(Тип("СлужебнаяСущность_ЭлементКоллекцииКлючЗначение"), ОбъектМодели.МодельДанных());
		...
		КолонкаValue.ТипКолонки = Колонка.ТипЭлемента;
