...
Перем Соединение;
Перем КартаТипов;
Перем ТипыРасширяемыеТаблицами;
...
Процедура ПриСозданииОбъекта()
	...
	ТипыРасширяемыеТаблицами = ПолучитьТипыРасширяемыеТаблицами();
...
Процедура ИнициализироватьТаблицу(ОбъектМодели) Экспорт
	...
	Для Каждого Колонка Из КолонкиТаблицы Цикл
		...
		Если ТипыРасширяемыеТаблицами.Найти(Колонка.ТипКолонки) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого Колонка Из КолонкиТаблицы Цикл
		...
		ОбъектМоделиЭлементКоллекции = ПолучитьОбъектМоделиЭлементКоллекции(ОбъектМодели, Колонка);
		ИмяТаблицы = ОбъектМодели.ИмяТаблицы() + "_" + Колонка.ИмяКолонки;
		
		ИнициализироватьТаблицу(ОбъектМоделиЭлементКоллекции);
...
Процедура Сохранить(ОбъектМодели, Сущность) Экспорт
	...
	Иначе
		Для Каждого ДанныеОКолонке Из КолонкиТаблицы Цикл
			...
			Если ТипыРасширяемыеТаблицами.Найти(ДанныеОКолонке.ТипКолонки) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
	...
	Для Каждого ДанныеОКолонке Из КолонкиТаблицы Цикл
		...
		ОбъектМоделиЭлементКоллекции = ПолучитьОбъектМоделиЭлементКоллекции(ОбъектМодели, ДанныеОКолонке);
		...
		УдалитьСтрокиВТаблице(ОбъектМоделиЭлементКоллекции, МассивОтборов);
		
		РасширяемоеПоле = ОбъектМодели.ПолучитьЗначениеПоля(Сущность, ДанныеОКолонке.ИмяПоля);
		
		Если НЕ ЗначениеЗаполнено(РасширяемоеПоле) Тогда
			Продолжить;
		КонецЕсли;
		
		КоллекцияСущностей = ОбъектМоделиЭлементКоллекции.ПреобразоватьКоллекциюКСущностям(
			РасширяемоеПоле,
			Сущность
		);
		
		Для Каждого ЭлементКоллекции Из КоллекцияСущностей Цикл
			Сохранить(ОбъектМоделиЭлементКоллекции, ЭлементКоллекции);
		КонецЦикла;
	...
	Для Каждого СтрокаИзБазы Из Результат Цикл
		...
		Для Каждого Колонка Из Колонки Цикл
			...
			ЭлементОтбора = Новый ЭлементОтбора(
				"ref",
				ВидСравнения.Равно,
				ЗначенияКолонок[ОбъектМодели.Идентификатор().ИмяКолонки]
			);

			МассивОтборов = Новый Массив;
			МассивОтборов.Добавить(ЭлементОтбора);
			ЗначениеКолонки = НайтиСтрокиВТаблице(ОбъектМоделиЭлементКоллекции, МассивОтборов);		
			
			ЗначениеКолонки = ОбъектМоделиЭлементКоллекции.ПреобразоватьКоллекциюСущностейККоллекции(ЗначениеКолонки, Колонка);
...
Функция ПолучитьТипыРасширяемыеТаблицами()
	Типы = Новый Массив;
	Типы.Добавить(ТипыКолонок.Массив);
	Типы.Добавить(ТипыКолонок.Структура);
...
Функция ПолучитьТипКолонкиСУБД(ОбъектМодели, КолонкаМодели)
	...
	ИначеЕсли ТипыКолонок.ЭтоКоллекция(КолонкаМодели.ТипКолонки) Тогда
		ОбъектМоделиЭлементКоллекции = ОбъектМодели.ПолучитьОбъектМоделиЭлементКоллекции(КолонкаМодели);
		ТипКолонкиСУБД = КартаТипов.Получить(ОбъектМоделиЭлементКоллекции.Идентификатор().ТипКолонки);
...
Функция ПолучитьОбъектМоделиЭлементКоллекции(РодительскийОбъектМодели, ДанныеОКолонке)
