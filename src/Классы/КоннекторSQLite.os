...
Процедура ИнициализироватьТаблицу(ОбъектМодели) Экспорт
	...
	Для Каждого Колонка Из КолонкиТаблицы Цикл
		...
		ОбъектМоделиЭлементКоллекции = ОбработкаКоллекций.ПолучитьОбъектМоделиДляКоллекции(ОбъектМодели, Колонка);
...
Функция НайтиСтрокиВТаблице(ОбъектМодели, Знач Отбор) Экспорт
	...
	Для Каждого СтрокаИзБазы Из Результат Цикл
		...
		Для Каждого Колонка Из Колонки Цикл
			...
			ЗначениеКолонки = ОбработкаКоллекций.ПреобразоватьКоллекциюСтрокККоллекции(ЗначениеКолонки, Колонка);
...
Функция ПолучитьТипКолонкиСУБД(ОбъектМодели, КолонкаМодели)
	...
	ИначеЕсли ТипыКолонок.ЭтоКоллекция(КолонкаМодели.ТипКолонки) Тогда
		ОбъектМоделиЭлементКоллекции = ОбработкаКоллекций.ПолучитьОбъектМоделиДляКоллекции(КолонкаМодели);
