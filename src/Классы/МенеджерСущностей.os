...
// Выполняет миграции схемы БД при инициализации
Перем МиграторСхемы;
...
Процедура ПриСозданииОбъекта(Знач ТипКоннектора, Знач СтрокаСоединения = "", Знач ППараметрыКоннектора = Неопределено)
	...
	МиграторСхемы = Новый МиграторСхемы(ЭтотОбъект);
...
//
// Для коннектора, реализующего интерфейс АбстрактныйКоннекторМиграций, выполняется
// автоматическая миграция схемы БД к состоянию модели данных (см. МиграторСхемы).
// Для прочих коннекторов таблицы инициализируются через ИнициализироватьТаблицу.
//
Процедура Инициализировать() Экспорт

	РаботаСКоннекторами.ОткрытьКоннектор(Коннектор, СтрокаСоединенияКоннектора, ПараметрыКоннектора);

	МиграторСхемы.ВыполнитьМиграцию();

КонецПроцедуры

// Возвращает мигратор схемы для настройки миграций до вызова метода Инициализировать:
// установки версии схемы приложения, регистрации обработчиков миграции,
// управления удалением лишних колонок.
//
//  Возвращаемое значение:
//   МиграторСхемы - Мигратор схемы данного менеджера сущностей.
//
Функция ПолучитьМиграторСхемы() Экспорт
	Возврат МиграторСхемы;
КонецФункции

// Устанавливает версию схемы данных приложения и включает хранение версии схемы
// в служебной таблице БД. См. МиграторСхемы.УстановитьВерсиюСхемы.
//
// Параметры:
