...
Процедура ДобавитьСущностьВМодель(ТипСущности) Экспорт
	...
	МодельСущности = Новый Структура;
	МодельСущности.Вставить("ИмяТаблицы", Строка(ТипСущности));

	Коннектор.ИнициализироватьТаблицу(МодельСущности);
КонецПроцедуры

Процедура Сохранить(Сущность) Экспорт
	ПроверитьЧтоКлассЯвляетсяСущностью(ТипЗнч(Сущность));

	Коннектор.Сохранить(Сущность);
КонецПроцедуры
