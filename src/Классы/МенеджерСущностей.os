...
// Возвращает коннектор текущего контекста исполнения.
//
// Коннектор берется из пула и закрепляется за контекстом до завершения транзакции, поэтому
// запросы, выполненные им напрямую, попадают в ту же транзакцию, что и операции менеджера.
//
//  Возвращаемое значение:
...
Функция ПолучитьКоннектор() Экспорт
	Возврат ПулСоединений.КоннекторТекущегоКонтекста();
КонецФункции
