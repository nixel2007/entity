...
//
Функция Получить(ТипСущности, Знач ОпцииПоиска = Неопределено, Соединение = Неопределено) Экспорт
	ОбъектМодели = МодельДанных.Получить(ТипСущности);
	ПулСущностей = ПолучитьПулСущностей(ТипСущности);
...
//
Функция ПолучитьОдно(ТипСущности, Знач ОпцииПоиска = Неопределено, Соединение = Неопределено) Экспорт
	ОбъектМодели = МодельДанных.Получить(ТипСущности);
	ПулСущностей = ПолучитьПулСущностей(ТипСущности);
	Если ОпцииПоиска = Неопределено Тогда
		ОпцииПоиска = Новый ОпцииПоиска;
...
// 
// Возвращаемое значение:
//   СоединениеСущности - Соединение для выполнения транзакционных операций из пула (если пул инициализирован)
...
Функция НачатьТранзакцию() Экспорт
	Если ПулСоединений <> Неопределено Тогда
		...
		Соединение = ПулСоединений.ПолучитьСоединение();
