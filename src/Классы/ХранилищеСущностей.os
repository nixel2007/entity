...
Процедура Сохранить(Сущность) Экспорт
	...
	Попытка
		РаботаСКоннекторами.Сохранить(Соединение.ПолучитьКоннектор(), ОбъектМодели, ПулСущностей, Сущность);
	Исключение
...
Функция Получить(Знач ОпцииПоиска = Неопределено) Экспорт
	...
	Попытка
		Результат = РаботаСКоннекторами.Получить(Соединение.ПолучитьКоннектор(), ОбъектМодели, ПулСущностей, ОпцииПоиска);
	Исключение
		ПулСоединений.Освободить();
		ВызватьИсключение;
	КонецПопытки;
...
Функция ПолучитьОдно(Знач ОпцииПоиска = Неопределено) Экспорт
	...
	Попытка
		Результат = РаботаСКоннекторами.ПолучитьОдно(
...
Процедура Удалить(Сущность) Экспорт
	...
	Попытка
		РаботаСКоннекторами.Удалить(Соединение.ПолучитьКоннектор(), ОбъектМодели, ПулСущностей, Сущность);
	Исключение
		ПулСоединений.Освободить();
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ЗафиксироватьТранзакцию() Экспорт
	...
	Соединение = ПулСоединений.СоединениеТекущегоКонтекста();
