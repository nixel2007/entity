...
Процедура ПриСозданииОбъекта(Знач ПОбъектМодели, Знач ПКоннектор, Знач ПСтрокаСоединения)
	...
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		...
		Для Каждого Колонка Из Колонки Цикл
			...
			Если Колонка.ТипКолонки = ТипыКолонок.Ссылка И ЗначениеЗаполнено(ЗначениеКолонки) Тогда
				ХранилищеСущностейСсылки = ХранилищаСущностей.Получить(Колонка.ТипСсылки, Коннектор, СтрокаСоединения);
...
Функция ПолучитьОдно(Знач Отбор = Неопределено) Экспорт
	...
	НайденныеСущности = Получить(ПередаваемыйОтбор);
