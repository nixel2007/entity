...
// Имя файла таблицы, блокировку которого пробует захватить фоновое задание
// в процедуре "ПроверитьЧтоБлокировкаТаблицыСвободна".
Перем ИмяФайлаПробы;
...
//
// Проба выполняется в фоновом задании: блокировка реентерабельна, и захват из того же
// контекста, что удерживает утечку, прошел бы успешно и ничего не выявил. Захват идет с
// таймаутом, поэтому утечка приводит к падению теста, а не к зависанию.
//
// Параметры:
...
Процедура ПроверитьЧтоБлокировкаТаблицыСвободна(ОбъектМодели, Пояснение)
	...
	ИмяФайлаПробы = ОбъединитьПути(СтрокаСоединения, ОбъектМодели.ИмяТаблицы()) + ".json";

	Задание = ФоновыеЗадания.Выполнить(ЭтотОбъект, "ЗахватитьБлокировкуТаблицыВФоне", Новый Массив, Истина);
	Задание.ОжидатьЗавершения();

	ОшибкаЗахвата = "";
	Если Задание.ИнформацияОбОшибке <> Неопределено Тогда
		ОшибкаЗахвата = ПодробноеПредставлениеОшибки(Задание.ИнформацияОбОшибке);
	КонецЕсли;
...
// Захватывает и сразу освобождает блокировку файла таблицы. Запускается в фоновом задании
// процедурой "ПроверитьЧтоБлокировкаТаблицыСвободна".
//
Процедура ЗахватитьБлокировкуТаблицыВФоне() Экспорт

	Блокировка = БлокировкиЧтенияЗаписи.Получить(ИмяФайлаПробы);
	Блокировка.ЗахватитьЗапись(ТаймаутОжиданияБлокировки());
...
// Таймаут ожидания захвата блокировки в проверках ее освобождения.
//
// Возвращаемое значение:
//   Число - Таймаут в миллисекундах.
//
Функция ТаймаутОжиданияБлокировки()
	Возврат 1000;
КонецФункции
