...
&Тест
Процедура ГенерацияПродолжаетсяПослеЯвногоИдентификатора() Экспорт
	...
	// Тогда
	Ожидаем.Что(Сущность.Идентификатор, "Генерация продолжает явно заданный идентификатор").Равно(6);

	НайденныеСтроки = Коннектор.НайтиСтрокиВТаблице(ОбъектМодели, Новый ОпцииПоиска());
	Ожидаем.Что(НайденныеСтроки, "Обе строки сохранены").ИмеетДлину(2);

КонецПроцедуры

&Тест
Процедура ГенерацияНеВыдаетИдентификаторУдаленнойСтроки() Экспорт
	...
	// Тогда
	Ожидаем.Что(Сущность.Идентификатор, "Идентификатор удаленной строки не выдается повторно").Равно(2);
...
Процедура СущностьМожетЗаписатьСебя() Экспорт
	...
	ИсточникТеста = Новый ИсточникДанных("Тесты", Тип(ТипКоннектора));
...
Процедура РаботаСИменованнымПолем() Экспорт
	// Дано
	ИсточникТеста = Новый ИсточникДанных("Тесты", Тип("КоннекторInMemory"));
	МенеджерСущностей = Новый МенеджерСущностей(ИсточникТеста);
...
// Операции идут под блокировкой общих данных подключения. Если ошибка оставит блокировку
// захваченной, следующая операция встанет насмерть, поэтому после каждой ошибки проверяется,
// что коннектор продолжает работать.
//
&Тест
Процедура ОшибкаОперацииНеУдерживаетБлокировку() Экспорт

	МодельДанных = Новый МодельДанных();
	ОбъектМодели = МодельДанных.СоздатьОбъектМодели(Тип("СущностьСИменованнымПолем"));

	Сущность = Новый СущностьСИменованнымПолем;
	Сущность.Имя = "имя";

	ПараметрыСущности = Новый Массив;
	ПараметрыСущности.Добавить(ОбъектМодели);
	ПараметрыСущности.Добавить(Сущность);

	ПараметрыПоиска = Новый Массив;
	ПараметрыПоиска.Добавить(ОбъектМодели);
	ПараметрыПоиска.Добавить(Новый ОпцииПоиска());

	// Таблица не инициализирована, поэтому каждая операция обязана выбросить исключение.
	Ожидаем.Что(Коннектор, "Сохранение без таблицы").Метод("Сохранить", ПараметрыСущности).ВыбрасываетИсключение();
	Ожидаем.Что(Коннектор, "Удаление без таблицы").Метод("Удалить", ПараметрыСущности).ВыбрасываетИсключение();
	Ожидаем.Что(Коннектор, "Поиск без таблицы").Метод("НайтиСтрокиВТаблице", ПараметрыПоиска).ВыбрасываетИсключение();
	Ожидаем.Что(Коннектор, "Удаление строк без таблицы")
		.Метод("УдалитьСтрокиВТаблице", ПараметрыПоиска)
	...
	НайденныеСтроки = Коннектор.НайтиСтрокиВТаблице(ОбъектМодели, Новый ОпцииПоиска());
	Ожидаем.Что(НайденныеСтроки, "Коннектор работает после ошибок").ИмеетДлину(1);
