...
Процедура Сохранить() Экспорт
	...
	ТекстЗапроса = СтрШаблон(
		"SELECT * FROM %1 WHERE ref = %2", 
		ОбъектМодели.ИмяТаблицы() + "_МассивСсылок",
		Сущность.Целое
	);
	РезультатЗапроса = Коннектор.ВыполнитьЗапрос(ТекстЗапроса);
...
Процедура ПолучитьЗначенияКолонокСущности() Экспорт
	...
	Ожидаем.Что(ЗначенияКолонок.Получить("Массив"), "ЗначенияКолонок.Массив проинициализировано массивом").ИмеетТип("Массив");
	Ожидаем.Что(ЗначенияКолонок.Получить("Структура"), "ЗначенияКолонок.Структура проинициализировано структурой").ИмеетТип("Структура");
	Ожидаем.Что(ЗначенияКолонок.Получить("Массив"), "ЗначенияКолонок.Массив имеет корректное количество элементов").ИмеетДлину(ЗависимыйМассив.Количество());
	Ожидаем.Что(ЗначенияКолонок.Получить("Структура"), "ЗначенияКолонок.Структура имеет корректное количество элементов").ИмеетДлину(ЗависимаяСтруктура.Количество());
	Ожидаем.Что(ЗначенияКолонок.Получить("МассивСсылок"), "ЗначенияКолонок.МассивСсылок проинициализированно массивом").ИмеетТип("Массив");
	Ожидаем.Что(ЗначенияКолонок.Получить("МассивСсылок"), "ЗначенияКолонок.МассивСсылок имеет корректное количество элементов").ИмеетДлину(МассивСсылок.Количество());
	Ожидаем.Что(ЗначенияКолонок.Получить("МассивСсылок")[0], "ЗначенияКолонок.МассивСсылок содержит идентификатор зависимой сущности").Равно(ЗависимаяСущность.Целое);
КонецПроцедуры
