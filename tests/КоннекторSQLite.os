...
// ИсточникДанных - Источник менеджера набора: создается в подготовке, закрывается после теста.
Перем Источник;
Перем Коннектор;

&ВключенЕсли("() -> ЗапускатьТестыSQLite()")
&ТестовыйНабор
Процедура ПриСозданииОбъекта()
КонецПроцедуры

// Определяет, доступна ли в текущем окружении база SQLite.
//
// Возвращаемое значение:
//   Булево - Истина, если набор нужно выполнять.
//
Функция ЗапускатьТестыSQLite() Экспорт
	Возврат ТестовыеУтилиты.ПолучитьПеременнуюСредыИлиЗначение("TESTRUNNER_RUN_SQLITE_TESTS", "true") = "true";
КонецФункции
...
Процедура ПередЗапускомТеста() Экспорт
	...
	Источник = Новый ИсточникДанных("Тесты", Тип("КоннекторSQLite"), СтрокаСоединения);
	// oneunit исполняет подготовку и тест в разных потоках: источник не должен закрыться с потоком подготовки.
	...
	// Тесты коннектора работают с собственным экземпляром: соединения из пула принадлежат пулу.
...
// Описание соединения - экспортный метод интерфейса НаблюдаемыйКоннектор, и позвать его могут
// раньше открытия. Отдавать в полях структуры Неопределено вместо пустой строки в таком случае
// незачем: поля описания - строковые.
&Тест
Процедура ОписаниеНеоткрытогоКоннектораБезНеопределено() Экспорт

	Описание = Новый КоннекторSQLite().ОписаниеСоединения();

	Ожидаем.Что(Описание.БазаДанных, "База данных неоткрытого коннектора - пустая строка").Равно("");
