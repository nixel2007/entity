...
Процедура ПолучитьСущности() Экспорт
	...
	Ожидаем.Что(ПолученнаяСущность.МассивСсылок[0].Целое, "ПолученнаяСущность.МассивСсылок содержит ссылку на зависимую сущность").Равно(ЗависимаяСущность.Целое);
