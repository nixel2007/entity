...
Процедура СозданиеТаблицыПоКлассуМодели() Экспорт
	Результат = ТестовыеУтилиты.ВыполнитьЗапрос(МенеджерСущностей, "SELECT * FROM Авторы");
...
// Активная запись строится одним построителем независимо от того, кто ее создал: и элемент
// менеджера, и элемент хранилища работают через хранилище своего типа.
&Тест
Процедура ЭлементМенеджераРаботаетЧерезХранилищеСвоегоТипа() Экспорт

	ХранилищеАвторов = МенеджерСущностей.ПолучитьХранилищеСущностей(Тип("Автор"));

	ЭлементМенеджера = МенеджерСущностей.СоздатьЭлемент(Тип("Автор"));
	ЭлементХранилища = ХранилищеАвторов.СоздатьЭлемент();

	Рефлектор = Новый Рефлектор();

	Ожидаем.Что(
		Рефлектор.ПолучитьСвойство(ЭлементХранилища, "_ХранилищеСущностей") = ХранилищеАвторов,
		"Элемент хранилища работает через это хранилище"
	).ЭтоИстина();

	Ожидаем.Что(
		Рефлектор.ПолучитьСвойство(ЭлементМенеджера, "_ХранилищеСущностей") = ХранилищеАвторов,
		"Элемент менеджера работает через то же хранилище"
	).ЭтоИстина();
...
Процедура СохранениеКоллекций() Экспорт
	...
	РезультатЗапроса = ТестовыеУтилиты.ВыполнитьЗапрос(МенеджерСущностей, ТекстЗапроса);
